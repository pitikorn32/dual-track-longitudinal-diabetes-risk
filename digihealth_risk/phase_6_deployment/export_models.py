"""Phase 6 deployment model export — dual-track, thesis-aligned.

Trains and exports 30 model artifacts (15 per track × 5 horizons × 3 history
windows) for the digihealth_risk FastAPI deployment.

The two tracks reflect the dual-output architecture argued in the thesis
(`docs/thesis/06_DISCUSSION.md` §6.4):

    Screening track (`/predict`)       — pure-prediction winners per horizon
                                          (thesis §5.1 final leaderboard).
    Intervention track (`/predict/...`) — monotonic intervention-safe winners
                                          per horizon (thesis §5.4).
...
* The thesis screening winner at N=5 is GEE. statsmodels GEE estimators do not
  serialize cleanly via joblib because prediction requires reconstructing the
  full design matrix from the fitted Family/Link objects. Logistic at N=5,M=5
  achieves PR-AUC 0.5248 vs GEE 0.5282 (thesis §5.1) — a 0.0034 gap — and is
  pure scikit-learn, so it is substituted here for deployability. The
  registry records the substitution explicitly.
...
`models_no_year/`, `model_registry_no_year.json`,
...
from digihealth_risk.phase_5.train_monotonic_xgboost import (  # noqa: E402
    BASE_MONOTONE_RULES,
...
def fit_and_save(track: str, horizon: int, history: int) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    registry_entry: dict[str, Any] = {
        ...
        "model_path": str(model_path(track, family, horizon, history).relative_to(ROOT)),
