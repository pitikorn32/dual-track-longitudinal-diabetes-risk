"""Phase 6 v2 monotonic-vs-unconstrained CatBoost ablation.

This mirrors the Phase 6 XGBoost and LightGBM ablations so intervention-ready
CatBoost can be compared against:
...
MODEL_DIR = OUT_DIR / "models_v2_catboost_ablation"
...
def model_key(variant: str, horizon: int, history_years: int) -> str:
    return f"phase6_v2_{variant}_catboost_n{horizon}_m{history_years}"


def model_name(variant: str) -> str:
    return f"{variant}_catboost_v2"


def build_catboost(*, constraints: tuple[int, ...] | None) -> CatBoostClassifier:
    kwargs: dict[str, Any] = {
        "iterations": 400,
        "depth": 4,
        "learning_rate": 0.03,
...
def fit_variant(train_df: pd.DataFrame, *, variant: str, history_years: int) -> dict[str, Any]:
    ...
    model = build_catboost(constraints=constraints if variant == "monotonic" else None)
...
def compare_against_xgb(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    cat_best = (
        ...
        .rename(
            columns={
                "model_key": "cat_model_key",
                "model_name": "cat_model_name",
                "history_years": "cat_history_years",
                "roc_auc": "cat_roc_auc",
                "pr_auc": "cat_pr_auc",
                "brier": "cat_brier",
                "recall": "cat_recall",
    ...
    comparison["cat_minus_xgb_pr_auc"] = comparison["cat_pr_auc"] - comparison["xgb_pr_auc"]
    comparison["cat_minus_xgb_roc_auc"] = comparison["cat_roc_auc"] - comparison["xgb_roc_auc"]
    comparison["cat_minus_xgb_recall"] = comparison["cat_recall"] - comparison["xgb_recall"]
    return comparison
...
def compare_safety_against_xgb(safety_summary_df: pd.DataFrame, comparison_df: pd.DataFrame) -> pd.DataFrame:
    ...
    chosen = comparison_df[["horizon_years", "cat_history_years", "xgb_history_years"]].copy()
    cat = safety_summary_df.merge(
        chosen.rename(columns={"cat_history_years": "history_years"}),
        on=["horizon_years", "history_years"],
        how="inner",
    )
    cat = cat[cat["variant"].eq("monotonic")].rename(
        columns={
            "history_years": "cat_history_years",
            "directionally_correct_rate": "cat_directionally_correct_rate",
            "unexpected_increase_rate": "cat_unexpected_increase_rate",
            "mean_delta_score": "cat_mean_delta_score",
            "worst_positive_delta_score": "cat_worst_positive_delta_score",
        }
    )
    ...
    result["cat_minus_xgb_directionally_correct_rate"] = (
        result["cat_directionally_correct_rate"] - result["xgb_directionally_correct_rate"]
    )
    return result
...
def write_report(
    ...
    lines = [
        "# Phase 6 v2 Monotonic CatBoost Ablation Report",
        "",
        "## Scope",
        "Benchmarks monotonic CatBoost against an otherwise identical unconstrained CatBoost. "
        ...
        markdown_table(
            xgb_comparison_df[
                [
                    ...
                    "cat_model_key",
                    "cat_pr_auc",
                    "cat_minus_xgb_pr_auc",
                ]
            ]
        ),
        "",
        "## CatBoost Intervention Safety Summary",
        markdown_table(
            best_safety_df[
                [
                    "variant",
        ...
        "",
        "## Monotonic CatBoost vs Monotonic XGBoost Safety",
        markdown_table(
            xgb_safety_df[
                [
                    "horizon_years",
                    ...
                    "xgb_directionally_correct_rate",
                    "cat_history_years",
                    "cat_directionally_correct_rate",
                    "cat_minus_xgb_directionally_correct_rate",
                    "xgb_unexpected_increase_rate",
                    "cat_unexpected_increase_rate",
                ]
            ]
        ),
        "",
...
def main() -> None:
    ...
    for history_years in HISTORY_OPTIONS:
        for horizon in HORIZONS:
            for variant in VARIANTS:
                print(f"Training {variant} CatBoost v2 N={horizon}, M={history_years}")
    ...
    metrics_df.to_csv(OUT_DIR / "phase_6_v2_catboost_ablation_metrics.csv", index=False)
    predictions_df.to_csv(
        OUT_DIR / "phase_6_v2_catboost_ablation_test_predictions.csv", index=False
    )
    constraints_df.to_csv(
        OUT_DIR / "phase_6_v2_catboost_ablation_constraints.csv", index=False
    )
    scenario_summary_df.to_csv(
        OUT_DIR / "phase_6_v2_catboost_ablation_scenario_summary.csv", index=False
    )
    safety_summary_df.to_csv(
        OUT_DIR / "phase_6_v2_catboost_ablation_safety_summary.csv", index=False
    )
    leaderboard_comparison_df.to_csv(
        OUT_DIR / "phase_6_v2_catboost_ablation_vs_leaderboard.csv", index=False
    )
    xgb_comparison_df.to_csv(
        OUT_DIR / "phase_6_v2_catboost_ablation_vs_monotonic_xgboost.csv", index=False
    )
    ...
    (OUT_DIR / "phase_6_v2_catboost_ablation_report.md").write_text(
