"""Run Phase 1 v2 GEE across horizon/history tables.

This wrapper mirrors `phase_1_gee_horizon_history_grid.py` but targets the
v2 feature-engineered GEE implementation.
...
GEE_SCRIPT = ROOT / "digihealth_risk" / "phase_1" / "gee.py"
DEFAULT_HORIZONS = [1, 2, 3, 4, 5]
DEFAULT_HISTORY = 5


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 1 v2 GEE for multiple N/M tables.")
    parser.add_argument(
        "--horizons",
        nargs="+",
        type=int,
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Re-run GEE even when the metrics output already exists.",
    )
...
def output_prefix(horizon: int, history_years: int) -> str:
    return f"phase_1_v2_gee_horizon_{horizon}_history_{history_years}"


def run_gee(input_path: Path, prefix: str) -> None:
    command = [
        sys.executable,
        str(GEE_SCRIPT),
        "--input-path",
...
def main() -> None:
    ...
    for horizon in sorted(set(args.horizons)):
        ...
        if metrics_path.exists() and not args.force:
            print(f"Skipping existing v2 GEE output N={horizon}, M={args.history_years}: {metrics_path}")
            continue

        print(f"Running v2 GEE N={horizon}, M={args.history_years}")
        run_gee(input_path, prefix)
